...
@implementation AttributedMarkupAction

+ (NSArray *)styledActionWithAction:(void (^)())actionBlock {

    AttributedMarkupAction *aciton = [[AttributedMarkupAction alloc] init];
    aciton.actionBlock = actionBlock;
    
    return @[@{NSStringFromClass([AttributedMarkupAction class]):aciton},kBlockLinkStyle];
}
...
@implementation ClickEventLabel
...
- (void)labelClickEvent:(UITapGestureRecognizer*)gesture {
    ...
    if (gesture.state == UIGestureRecognizerStateRecognized) {
        ...
        AttributedMarkupAction* action = attributes[NSStringFromClass([AttributedMarkupAction class])];
        if (action) {
            action.actionBlock();
        }
    }
